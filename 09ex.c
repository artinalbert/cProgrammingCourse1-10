...
uint32_t* return_pointer(uint32_t x) {
    uint32_t* temp = malloc(sizeof x);
    //position where temp points to (shown with star)
    *temp = x;
    return temp;
}
...
der *nicht* in die Speicherregion eines anderen Programms führt - >> will instead return the pointer in that memory space
 casted implicitely to whatever the type of returned value was supposed to be!
...
//    for (PileOfPancakes cake = p; cake != NULL ; cake = cake.further_layers) {
//
//    //free(p.further_layers(i));
//    }

    while (p.further_layers != NULL){
        PileOfPancakes toFree = p;
        //
        p = *p.further_layers;
        free(&toFree);
    }
