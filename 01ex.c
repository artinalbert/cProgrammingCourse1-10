...
int rotation(int a, int b, int c) {
    ...
    */
    /* BEGIN CHANGES */
    int temp = a; // Temporäre Variable, um den Wert von `a` zu speichern
    a = b;        // Wert von `b` wird `a` zugewiesen
    b = c;        // Wert von `c` wird `b` zugewiesen
    c = temp;     // Ursprünglicher Wert von `a` (gespeichert in `temp`) wird `c` zugewiesen

    /* END CHANGES */

    // Optional: Geben Sie die aktualisierten Werte zurück, um das Ergebnis zu überprüfen
    printf("a = %d, b = %d, c = %d\n", a, b, c);
    //?
...
int for_loop(int n) {
    ...
    `n` ist nie kleiner als `1`.
     ...
     -Initialisierung (wird nur einmal am Anfang ausgeführt): int i = 1;
    -Bedingungsüberprüfung (vor jedem Schleifendurchlauf): i <= n;
    -Ausführung des Schleifeninhalts (falls die Bedingung wahr ist): result = result * i;
    -Nachbearbeitung (am Ende jedes Schleifendurchlaufs): i++

    */
    //? when does the condition get executed? in the beginning or end of each loop?
    /* BEGIN CHANGES */
    for(int i = 1; i <= n; i++) {
        result = result * i;
    }
